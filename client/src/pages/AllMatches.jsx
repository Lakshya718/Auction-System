      ...
      {matches.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {matches.map((match) => (
            <div
              key={match._id}
              className="bg-gray-800 rounded-lg shadow-lg p-6 transform transition duration-300 hover:scale-105 hover:shadow-2xl"
            >
              ...
              <p
                className={`text-lg font-semibold ${match.matchStatus === 'completed' ? 'text-green-500' : match.matchStatus === 'in-progress' ? 'text-yellow-500' : 'text-blue-400'}`}
              >
