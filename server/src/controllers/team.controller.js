...
export const getTeam = async (req, res) => {
  try {
    ...
    const team = await Team.findById(id).populate("owner", "name email");
    ...
    const auctions = await Auction.find({
      $or: [{ "players.soldTo": id }, { "retainedPlayers.team": id }],
    }).populate(
      "players.player retainedPlayers.player",
      "playerName playerRole battingStyle bowlingStyle stats"
    );
...
export const getMyTeam = async (req, res) => {
  try {
    if (req.user.role !== "team_owner") {
      return res
        .status(403)
        .json({ error: "Only team owners can access this endpoint" });
    }
