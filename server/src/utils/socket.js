  ...
  io.on("connection", (socket) => {
    ...
    socket.on("join-auction", async ({ auctionId }) => {
      ...
      socket.emit("joined-auction", {
        auctionId,
        message: `Joined auction ${auction.tournamentName}`,
      });
    ...
    socket.on(
      "sell-player",
      async ({ auctionId, playerId, teamId, amount }) => {
        try {
          if (socket.user.role !== "admin") {
            return socket.emit("error", {
              message: "Only admins can sell players",
            });
          }
          ...
          if (
            ...
            return socket.emit("error", {
              message: "Invalid auction, player, or team ID",
            });
          }
          ...
          if (!player || player.status !== "available") {
            return socket.emit("error", {
              message: "Player is not available for sale",
            });
          }
          ...
          if (!teamBudget || teamBudget.remainingBudget < amount) {
            return socket.emit("error", {
              message: "Insufficient team budget",
            });
          }
          ...
          // Audit log
          console.log(
            `Player sold: ${playerId} to team ${teamId} for ${amount} in auction ${auctionId}`
          );
        } catch (error) {
    ...
    socket.on("join-tournament", async ({ tournamentId }) => {
      ...
      socket.emit("joined-tournament", {
        tournamentId,
        message: `Joined tournament ${auction.tournamentName}`,
      });
    });
    ...
    socket.on("place-bid", async ({ auctionId, playerId, amount, teamId }) => {
      try {
        ...
        if (
          socket.user.role !== "team_owner" ||
          !(await Team.findOne({ _id: teamId, owner: socket.user._id }))
        ) {
          return socket.emit("error", {
            message: "Not authorized to bid for this team",
          });
        }
        ...
        if (amount < player.currentBid + auction.minBidIncrement) {
          return socket.emit("error", {
            message: `Bid must be at least ${
              player.currentBid + auction.minBidIncrement
            }`,
          });
        }
        ...
        // Audit log
        console.log(
          `Bid placed: ${socket.user.email} bid ${amount} for player ${playerId} in auction ${auctionId}`
        );
      } catch (error) {
    ...
    socket.on("leaveMatch", (matchId) => {
      if (matchId) {
        ...
        socket.emit("leftMatch", { matchId, message: `Left match ${matchId}` });
